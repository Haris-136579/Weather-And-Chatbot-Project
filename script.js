...
document.getElementById('chat-form').addEventListener('submit', async function (e) {
    ...
    } else if (userInput.toLowerCase().includes('temperature on')) {
        response = getTemperatureByDate(userInput);
    } else {
        try {
...
function getTemperatureByDate(userInput) {
    ...
    const dateMatch = userInput.match(/(?:temperature on|for)\s*(\d{1,2}[-/]\d{1,2}[-/]\d{2,4}|\w+\s+\d{1,2}|\d{1,2}\s+\w+\s+\d{2,4})/i);
    if (!dateMatch) return "Please provide a valid date to check the temperature.";

    const userDate = new Date(dateMatch[1]);
    const dateString = userDate.toISOString().split('T')[0]; // Format to YYYY-MM-DD

    // Find the matching forecast data
    const matchingForecast = forecastData.list.find(item => {
        const forecastDate = new Date(item.dt * 1000);
        return forecastDate.toISOString().split('T')[0] === dateString; // Compare date strings
    });

    if (matchingForecast) {
        const temperature = matchingForecast.main.temp;
        const condition = matchingForecast.weather[0].description;
        const units = document.getElementById('units').value === 'metric' ? '°C' : '°F';
        return `The temperature on ${userDate.toLocaleDateString()} is ${temperature.toFixed(2)}${units} with ${condition}.`;
    } else {
        return `Sorry, I don't have weather data for the date ${userDate.toLocaleDateString()}.`;
    }
}
